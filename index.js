...
ws.onmessage = async (event) => {
    try {
        ...
        let sma5 = SMA.calculate({ period: 201, values: closeValues });
        let media200 = sma5[sma5.length - 2];
        ...
        const macd = result[result.length - 2].MACD;
        const sinal = result[result.length - 2].signal;
        const histograma = result[result.length - 2].histogram;0
        ...
        if ( ultima_vela > media200 && macd  > sinal && histograma > 0 && Operacao_aberta === 0) {
            console.log("Compra")
            Operacao_aberta = 1
